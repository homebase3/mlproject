...
3.  Pass regressor to Boruta
4.  Set ranges for hyperparameters
5.  Set up cross-validation scoring
6.  Functionalize steps 1-5 and pass to Optuna
7.  Get best parameters for each model from Optuna
...
OneHotEncoder().fit_transform(X) #need to do encode categorical variables for XGBoost

xgb_model = xgb.XGBRegressor(random_state=42)
feat_selector = BorutaPy(xgb_model, n_estimators='auto', verbose=2, random_state=1)
transformed_selector = TransformedTargetRegressor(regressor = feat_selector, transformer = Qu )
```
