...
This variable selection returns \~16% of the total dummified variables. Interestingly though, \~% of the underlying variables are included. In effect, most of the dimensionality reduction comes from reducing dummified options of categorical variables.
...
transformed_xgb_model = TransformedTargetRegressor(regressor = xgb_model, func = np.log1p, inverse_func = np.expm1)
cross_val_score(transformed_xgb_model,X_xgb,y, cv = 5, scoring = 'neg_root_mean_squared_error')
...
```{python}
lgb_model = lgb.LGBMRegressor(**study2.best_params)
transformed_lgb_model = TransformedTargetRegressor(regressor = lgb_model, func = np.log1p, inverse_func = np.expm1)
np.mean(cross_val_score(transformed_lgb_model,X_cat,y, cv = 5, scoring = 'neg_root_mean_squared_error'))
```
...
cross_val_score(transformed_cat_model,X_cat,y, cv = 5, scoring = 'neg_root_mean_squared_error')
```
...
Below are the top 10 features from the LightGBM model
